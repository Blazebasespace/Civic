/*
  ...
  1. New Tables
    - `citizens` - User profiles and civic identity data
    - `proposals` - Governance proposals with voting mechanisms
    - `votes` - Individual vote records with validation
    - `activities` - Civic participation tracking
    - `ai_analyses` - AI Governor analysis results
    - `forum_posts` - Citizen forum discussions
    - `network_states` - Global network state data
  ...
  4. Sample Data
    - Pre-populated network states for testing
...
-- Add foreign key constraint for votes if it doesn't exist
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.table_constraints 
    WHERE constraint_name = 'votes_proposal_id_fkey'
  ) THEN
...
-- Add foreign key constraint for ai_analyses if it doesn't exist
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.table_constraints 
    WHERE constraint_name = 'ai_analyses_proposal_id_fkey'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_tables WHERE tablename = 'citizens' AND rowsecurity = true
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_tables WHERE tablename = 'proposals' AND rowsecurity = true
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_tables WHERE tablename = 'activities' AND rowsecurity = true
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_tables WHERE tablename = 'ai_analyses' AND rowsecurity = true
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_tables WHERE tablename = 'forum_posts' AND rowsecurity = true
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_tables WHERE tablename = 'network_states' AND rowsecurity = true
  ) THEN
...
-- Insert sample network states (only if they don't exist)
INSERT INTO network_states (name, flag, population, gdp, governance_score, innovation_index, location, established, specialties, partnerships, color) 
SELECT * FROM (VALUES
  ('Infinita City', '🏙️', 45000, 2.3, 94, 87, 'Caribbean', '2023', '{"AI Governance", "Sustainable Tech", "Digital Finance"}', '{"Próspera", "Zuzalu"}', 'from-blue-500 to-cyan-500'),
  ('Próspera', '🏛️', 12000, 0.8, 91, 82, 'Honduras', '2020', '{"Charter City", "Economic Freedom", "Regulatory Innovation"}', '{"Infinita City", "Nueva Libertad"}', 'from-green-500 to-emerald-500'),
  ('Zuzalu', '🌊', 8500, 0.4, 88, 95, 'Montenegro', '2023', '{"Research", "Longevity", "Crypto Innovation"}', '{"Infinita City", "Digital Nomad Republic"}', 'from-purple-500 to-pink-500'),
  ('Nueva Libertad', '🗽', 23000, 1.2, 89, 78, 'Chile', '2022', '{"Mining Tech", "Renewable Energy", "Space Industry"}', '{"Próspera", "Aurora Station"}', 'from-orange-500 to-red-500'),
  ('Digital Nomad Republic', '🌐', 67000, 3.1, 85, 92, 'Distributed', '2021', '{"Remote Work", "Digital Services", "Global Mobility"}', '{"Infinita City", "Zuzalu"}', 'from-teal-500 to-blue-500'),
  ('Aurora Station', '🚀', 150, 0.05, 76, 98, 'Low Earth Orbit', '2024', '{"Space Governance", "Zero-G Manufacturing", "Orbital Research"}', '{"Nueva Libertad", "Digital Nomad Republic"}', 'from-indigo-500 to-purple-500')
) AS v(name, flag, population, gdp, governance_score, innovation_index, location, established, specialties, partnerships, color)
WHERE NOT EXISTS (
  SELECT 1 FROM network_states WHERE network_states.name = v.name
);